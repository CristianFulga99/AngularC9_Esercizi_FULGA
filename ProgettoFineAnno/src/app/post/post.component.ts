  ...
  commenta(commento: HTMLInputElement, post: Post){
    if(this.username != null){
      if(commento.value != ''){
        ...
        recensione.testo = commento.value;
        recensione.nomeUtente = this.nomeUtenteOnline;
        recensione.cognomeUtente = this.cognomeUtenteOnline;
        recensione.username = this.username;
        let date: Date = new Date();
        recensione.data = date.getHours() + ':' + date.getMinutes() + ', il ' + date.getDay() + '/' + date.getMonth() + '/' + date.getFullYear();
        post.commenti.push(recensione);
      } else {
